...
# Ejecutar resolución
sols = resolver_ftcs(u0, dx, dy, dt, pasos, alpha)
...
heat = ax.contourf(X, Y, u0, levels=30, cmap='inferno')
...
grafico = ax2.imshow(sols[0], cmap='inferno', origin='lower', extent=(0, Lx, 0, Ly), vmin=0, vmax=1)
cbar2 = fig2.colorbar(grafico, ax=ax2)
cbar2.set_label('Temperatura')
txt = ax2.text(0.05, 1.03, '', transform=ax2.transAxes)

ax2.set_title('Evolución de temperatura (FTCS)')
ax2.set_xlabel('x')
ax2.set_ylabel('y')
