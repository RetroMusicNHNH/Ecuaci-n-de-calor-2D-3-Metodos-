"""Ejemplo visual Crank-Nicolson para ecuación de calor 2D"""
...
dx = Lx / (N - 1)
dy = Ly / (N - 1)
dt = 0.9 * dx**2 / (4 * alpha) * 4   # Crank-Nicolson permite dt más grande
T_final = 0.1
...
# Ejecutar resolución
sols = resolver_cn(u0, dx, dy, dt, pasos, alpha)
...
heat = ax.contourf(X, Y, u0, levels=30, cmap='cool')
...
grafico = ax2.imshow(sols[0], cmap='cool', origin='lower', extent=(0, Lx, 0, Ly), vmin=0, vmax=1)
cbar2 = fig2.colorbar(grafico, ax=ax2)
cbar2.set_label('Temperatura')
txt = ax2.text(0.05, 1.03, '', transform=ax2.transAxes)

ax2.set_title('Evolución de temperatura (Crank-Nicolson)')
ax2.set_xlabel('x')
ax2.set_ylabel('y')
